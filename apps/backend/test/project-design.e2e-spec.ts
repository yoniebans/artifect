// test/product-design.e2e-spec.ts
...
describe('Product Design Project Type (e2e)', () => {
    ...
    // Product Design project type expected ID
    const PRODUCT_DESIGN_TYPE_ID = 2;

    // Artifact types specific to Product Design
    ...
    it('should create a User Research artifact for the Product Design project', async () => {
        const response = await request(app.getHttpServer())
            ...
            .send({
                ...
                artifact_type_name: DESIGN_ARTIFACT_TYPES.USER_RESEARCH
            })
            .expect(201);

        expect(response.body).toHaveProperty('artifact');
        expect(response.body.artifact).toHaveProperty('artifact_id');
        expect(response.body.artifact).toHaveProperty('artifact_type_name', DESIGN_ARTIFACT_TYPES.USER_RESEARCH);
    ...
    it('should reject creating a Vision Document (Software Engineering artifact) for a Product Design project', async () => {
        const response = await request(app.getHttpServer())
            ...
            .send({
                project_id: projectId,
                artifact_type_name: 'Vision Document' // This belongs to Software Engineering
            })
            .expect(400);
    ...
    it('should create a Design Brief artifact after User Research', async () => {
        const response = await request(app.getHttpServer())
            ...
            .send({
                project_id: projectId,
                artifact_type_name: DESIGN_ARTIFACT_TYPES.DESIGN_BRIEF
            })
            .expect(201);

        expect(response.body).toHaveProperty('artifact');
        expect(response.body.artifact).toHaveProperty('artifact_type_name', DESIGN_ARTIFACT_TYPES.DESIGN_BRIEF);
    });
    ...
    it('should create Wireframes artifact in the Concept phase', async () => {
        const response = await request(app.getHttpServer())
            ...
            .send({
                project_id: projectId,
                artifact_type_name: DESIGN_ARTIFACT_TYPES.WIREFRAMES
            })
            .expect(201);

        expect(response.body).toHaveProperty('artifact');
        expect(response.body.artifact).toHaveProperty('artifact_type_name', DESIGN_ARTIFACT_TYPES.WIREFRAMES);
    });
    ...
    it('should create Mockups artifact which depends on Wireframes', async () => {
        const response = await request(app.getHttpServer())
            ...
            .send({
                project_id: projectId,
                artifact_type_name: DESIGN_ARTIFACT_TYPES.MOCKUPS
            })
            .expect(201);

        expect(response.body).toHaveProperty('artifact');
        expect(response.body.artifact).toHaveProperty('artifact_type_name', DESIGN_ARTIFACT_TYPES.MOCKUPS);
    });
    ...
    it('should show the complete project with all artifacts organized by phases', async () => {
        ...
        expect(artifactTypeNames).toContain(DESIGN_ARTIFACT_TYPES.USER_RESEARCH);
        expect(artifactTypeNames).toContain(DESIGN_ARTIFACT_TYPES.DESIGN_BRIEF);
        expect(artifactTypeNames).toContain(DESIGN_ARTIFACT_TYPES.WIREFRAMES);
        expect(artifactTypeNames).toContain(DESIGN_ARTIFACT_TYPES.MOCKUPS);
        ...
        if (researchPhase) {
            ...
            expect(researchArtifactTypes).toContain(DESIGN_ARTIFACT_TYPES.USER_RESEARCH);
            expect(researchArtifactTypes).toContain(DESIGN_ARTIFACT_TYPES.DESIGN_BRIEF);
        }
        ...
        if (conceptPhase) {
            ...
            expect(conceptArtifactTypes).toContain(DESIGN_ARTIFACT_TYPES.WIREFRAMES);
            expect(conceptArtifactTypes).toContain(DESIGN_ARTIFACT_TYPES.MOCKUPS);
        }
    });
});